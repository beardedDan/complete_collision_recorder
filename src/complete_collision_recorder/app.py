...
def extract_cad_text():
    ...
    try:
        script_path = os.path.join(
            os.getcwd(), "extract_and_ingest_text_from_cad_pdf.py"
        )
...
def extract_oh1_text():
    ...
    try:
        script_path = os.path.join(
            os.getcwd(), "extract_and_ingest_text_from_oh1_pdf.py"
        )
        result = subprocess.run(
            ["python", script_path], capture_output=True, text=True
        )
...
def score_and_describe_text():
    ...
    try:
        # script_path = os.path.join(
        #     os.getcwd(), "score_and_describe_reports.py"
        # )
        # result = subprocess.run(['python', script_path], capture_output=True, text=True)
        result = subprocess.run(
            ["python", "score_and_describe_reports.py", "--testing"],
            capture_output=True,
            text=True,
        )
